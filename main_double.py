...
df = pd.read_excel(r'C:\Users\Администратор\PycharmProjects\kursach-vsp\ОценочныеКритерииПрочностиПути.xlsx', sheet_name="Вагон")
...
alarm = list(workbook_7.loc[workbook_7["Type"] == Type[0], 'P_ct'])[0]
...
workbook_8 = pd.read_excel(r'C:\Users\Администратор\PycharmProjects\kursach-vsp\РасчетныеХарактеристикиПути — копия.xlsx')
...
delta_t_p1_min_pryamo = min(sostavs[1].delta_t_p(), sostavs[5].delta_t_p())
delta_t_p0_min_crivaya = min(sostavs[3].delta_t_p(), sostavs[7].delta_t_p())
...
def indexxx_0():
    ...
    for index, sostav in enumerate(sostavs):
        if sostav.delta_t_p() == delta_t_p0_min_crivaya:
            return index
...
def indexxx_1():
    ...
    global sostav
    for index, sostav in enumerate(sostavs):
        if sostav.delta_t_p() == delta_t_p1_min_pryamo:
            return index
...
for i, sostav in enumerate(sostavs, start=1):
    ...
    sheet.cell(row=113, column=i, value=delta_t_p0_min_crivaya)
    sheet.cell(row=113, column=9, value="[∆t_р_min]Прямая")
    sheet.cell(row=114, column=i, value=round(sostavs[indexxx_0()].sigma_kp() * 1.3 + 25.2 * delta_t_p0_min_crivaya, 2))
    sheet.cell(row=114, column=9, value="нормальные суммарные напряжения(Прямая)")

    sheet.cell(row=115, column=i, value=delta_t_p1_min_pryamo)
    sheet.cell(row=115, column=9, value="[∆t_р_min]Кривая")
    sheet.cell(row=116, column=i, value=round(sostavs[indexxx_1()].sigma_kp() * 1.3 + 25.2 * delta_t_p1_min_pryamo, 2))
    ...
    sheet.cell(row=117, column=i, value=25.2 * delta_t_p0_min_crivaya)
    sheet.cell(row=118, column=i, value=25.2 * delta_t_p1_min_pryamo)
    ...
    sheet.cell(row=141, column=i, value=delta_t_p0_min_crivaya + round(P_norm0 / (25 * 2 * F), 2) - 10)
    sheet.cell(row=141, column=9, value="[T] прямая")
    sheet.cell(row=140, column=i, value=delta_t_p1_min_pryamo + round(P_norm1 / (25 * 2 * F), 2) - 10)
    ...
    t_max_zakr = min(t_min_min + delta_t_p0_min_crivaya, t_max_max)
    sheet.cell(row=142, column=i, value=t_max_zakr)
    sheet.cell(row=142, column=9, value="t_max_zakr")
    t_max_zakr_curve = min(t_min_min + delta_t_p1_min_pryamo, t_max_max)
    ...
    vvv = GrafVerTemp(t_max_max=t_max_max, t_min_min=t_min_min,
                      ...
                      t_y=min(t_y, t_y_curve), delta_t_p=min(delta_t_p1_min_pryamo, delta_t_p0_min_crivaya), t_opt=t_opt)
    vvv.grafic_maker_()
    ...
    aaa = TempDiagramm(t_max_max=t_max_max, t_min_min=t_min_min, t_y_curve=t_y_curve, curve=curve[1],
                       ...
                       t_max_zakr_curve=t_max_zakr_curve, t_y=t_y, delta_t_p1_min=delta_t_p1_min_pryamo,
                       delta_t_p0_min=delta_t_p0_min_crivaya)
    aaa.grafic_maker()
...
#zzz = Grafics_izgibov(sostavs=sostavs, k = k, l_i = l_i)
#zzz.grafic_mader_()
